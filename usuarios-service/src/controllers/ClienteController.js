...
export async function registrar(req, res) {
  try {
    ...
    await apiClient.enviarNotificacion(email, "Crea tu contraseña", `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 20px; border: 1px solid #202020ff; border-radius: 8px; background-color: #f9f9f9;">
...
export async function registrarClienteReferido(req, res) {
  try {
    const { DNI, documento_referido, nombre, telefono, email, edad, peso, altura } = req.body;

    if (!DNI || !nombre || !telefono || !email || !edad || !peso || !altura || !documento_referido) {
      return res.status(400).json({ message: "Todos los campos son obligatorios" });
    }
